...
test('Гениальность добавляет бонус к броску атаки', () => {
  ...
  const hit = multiWeaponAttack(ctx({
    rng: seqRng([0.5, 0.99]), targets: [{ ac: 16, hp: 30 }], mods: { genius: 3 },
  }), [0]);
  assert.deepEqual(hit, [[{ type: 'physical', amount: 13 }]]);
});

test('Кроличья лапка перебрасывает неудачную атаку один раз', () => {
  // первый бросок nat 2 (промах против ac 16), переброс nat 20 -> крит: d10+d10 =20 +3 =23
  const p = multiWeaponAttack(ctx({
    rng: seqRng([0.05, 0.99, 0.99, 0.99]), targets: [{ ac: 16, hp: 30 }], mods: { rabbitFoot: true },
  }), [0]);
  assert.deepEqual(p, [[{ type: 'physical', amount: 23 }]]);
});

test('преимущество на цели действует только по этой цели', () => {
  // Цель 0 скована (преимущество), цель 1 — нет. Против ac 18 при str 3 нужен nat>=15.
  // Порядок бросков: цель 0 берёт лучший из (0.5=11, 0.99=20) -> крит, кубы 0.99,0.99;
  // цель 1 кидает один д20 0.5 -> 11, промах.
  const p = multiWeaponAttack(ctx({
    rng: seqRng([0.5, 0.99, 0.99, 0.99, 0.5]),
    targets: [{ ac: 18, hp: 30, adv: true }, { ac: 18, hp: 30 }],
  }), [0, 1]);
  assert.deepEqual(p, [[{ type: 'physical', amount: 23 }], []]);
});

test('помеха на цели действует только по этой цели', () => {
  // Цель 0 под помехой: берёт худший из (0.99, 0.5) -> 11, против ac 12 при str 3 = 14, попал
  // но не крит, хотя выпала двадцатка. Цель 1 без помехи кидает один бросок.
  const p = multiWeaponAttack(ctx({
    rng: seqRng([0.99, 0.5, 0.99, 0.5, 0.99]),
    targets: [{ ac: 12, hp: 30, dis: true }, { ac: 12, hp: 30 }],
  }), [0, 1]);
  assert.deepEqual(p, [[{ type: 'physical', amount: 13 }], [{ type: 'physical', amount: 13 }]]);
});
